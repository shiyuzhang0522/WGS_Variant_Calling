...
def main():
    ...
    parser.add_argument(
        "--metadata",
        default="/public/home/hpc8301200407/XYCM_WGS/Variant_Calling/XYCM_WGS_sample_metadata.tsv",
    )
    parser.add_argument(
        "--ref-dir",
        default="/public/home/hpc8301200407/XYCM_WGS/Variant_Calling/Resource.bundle/GATK.hg38",
    )
    parser.add_argument(
        "--out-dir",
        default="/public/home/hpc8301200407/XYCM_WGS/Variant_Calling/WDL/inputs",
    )
    parser.add_argument("--workflow-name", default="XYCM_Germline_GVCF")
    ...
    if df["sample_uid"].duplicated().any():
        dup = df.loc[df["sample_uid"].duplicated(), "sample_uid"].tolist()
        raise ValueError(f"Duplicated sample_uid found: {dup[:10]}")

    prefix = args.workflow_name
    manifest_rows = []

    for _, row in df.iterrows():
        sample_uid = sanitize_id(row["sample_uid"])
