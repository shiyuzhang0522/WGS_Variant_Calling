...
##   4. Clean large intermediate BAM/BAI/MD5 files after success.
...
# Cromwell preserves relative output paths, so final outputs may be nested.
...
# Flatten final GVCF/TBI into FINAL_DIR, then remove nested duplicates.
...
if [[ "${RC}" -eq 0 && -s "${FINAL_GVCF}" && -s "${FINAL_TBI}" ]]; then
  ...
  echo "Cleaning large intermediate BAM/BAI/MD5 files from RUN_DIR..."
  if [[ -d "${RUN_DIR}/cromwell-executions" ]]; then
    find "${RUN_DIR}/cromwell-executions" \
      \( -name "*.bam" -o -name "*.bai" -o -name "*.bam.md5" \) \
